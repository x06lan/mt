...
void big_div(int *a,int *b,int *c){
	int i,j;
	int tem[90];
	memset(tem,0,90*sizeof(int));
	for(i=40;i>=0;i--){
		for(j=9;j>=0;j--){
			int tem2[100];
			memset(tem2,0,100*sizeof(int));
			int tem3[100];
			memset(tem3,0,100*sizeof(int));
			tem2[0]=j;
			tem[i]=1;
			big_mul(b,tem2,tem3);
			memset(tem2,0,100*sizeof(int));
...
int main() {
	...
	scanf("%s",&a);
	scanf("%s",&b);
	memset(x,0,90*sizeof(int));
	memset(y,0,90*sizeof(int));
	memset(z,0,90*sizeof(int));
	str_int(a,x);
	str_int(b,y);
	//big_mul(x,y,z);
	big_div(x,y,z);
